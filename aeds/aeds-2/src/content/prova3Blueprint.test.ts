import { prova3Blueprint } from './prova3Blueprint';
import { evaluateStep } from '../engine/evaluator';

test('prova 3 tem exatamente 3 questoes reais (avl, trie, hibrida/doidona)', () => {
  expect(prova3Blueprint.questions).toHaveLength(3);
  expect(prova3Blueprint.questions.map((q) => q.domainId)).toEqual(['avl', 'trie', 'doidona']);
});

test('todas as questoes tem visual de apoio e pelo menos 2 passos', () => {
  prova3Blueprint.questions.forEach((question) => {
...
test('a solucao modelo de cada passo de funcao satisfaz seus proprios fragmentos obrigatorios', () => {
  const functionSteps = prova3Blueprint.questions
...
test('a resposta modelo de cada pergunta de complexidade (gap) satisfaz seu proprio gabarito', () => {
  const gapSteps = prova3Blueprint.questions.flatMap((question) => question.steps).filter((step) => step.kind === 'gap');
